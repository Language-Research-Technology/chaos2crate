...
// own INPUT_PLUGINS, generated by scripts/select-plugins.mjs from the
// INPUT_PLUGINS env var) rather than hardcoded to c2c-plugins' own
// generic/docx pair — a build can select a different set (e.g. an external
// input mode instead, or alongside them). INPUT_MODE_LABELS/INPUT_MODE_HINTS
// only cover c2c-plugins' own built-in modes; any other selected mode falls
// back to its own plugin.name as its dropdown label, and contributes no
// hint text of its own (an external input-mode package can document its own
// UI elsewhere — its own README, or a settingsSchema fragment, the same way
// other plugins already do for their own options).
const INPUT_MODE_LABELS = { generic: "Generic folder of files", docx: "Structured Word documents (.docx)" };
const INPUT_MODE_HINTS = {
  generic: "Generic folder builds scan files into a repository crate.",
  docx: "Structured Word documents follow Heading 1/2/3 collection conventions instead. Optional transcript processing is available as a Build option for CAAT/AmAus-style .docx transcript files.",
};
const INPUT_MODE_KEYS = Object.keys(INPUT_PLUGINS);

// Shown in the Settings modal (accessed from the button next to Menu).
const CORE_SETTINGS_SCHEMA = [
  { key: "inputMode", type: "select", label: "Input type", default: INPUT_MODE_KEYS[0],
    options: INPUT_MODE_KEYS.map((mode) => ({ value: mode, label: INPUT_MODE_LABELS[mode] || INPUT_PLUGINS[mode].name })),
    hint: INPUT_MODE_KEYS.map((mode) => INPUT_MODE_HINTS[mode]).filter(Boolean).join(" ")
      || "Chooses which input-mode plugin parses the picked folder into a crate." },
...
function renderOptionGroupTiles(schema, tilesContainer, modalBody) {
  for (const opt of schema) {
    // A plain action tile — no on/off toggle, no modal-with-children; a
    // plugin that wants to run something immediately (open its own modal,
    // scan the folder, whatever) rather than configure a build-time option,
    // like every other tile here does. `run(runtime)` is the plugin's own
    // handler; runtime is {dirHandle, log} — read fresh at click time, not
    // captured at render time, since dirHandle changes with every folder
    // pick. The host has no idea what a given action tile actually does.
    if (opt.kind === "action") {
      ...
      desc.textContent = opt.hint || "";

      tile.append(head, desc);
      tile.addEventListener("click", () => opt.run({ dirHandle, log }));
...
async function run() {
  ...
  startBuildProgress();
  $("showHtmlBtn").classList.add("hidden"); $("showHtmlHint").classList.add("hidden"); buildHtml = null;
...
async function openBuild() {
  ...
  $("showHtmlHint").classList.add("hidden");
...
function isModalOpen() {
  const ids = [
    "modal", "settingsModal", "mergeMappingModal", "collectionLabelsModal", "optionGroupModal",
    "pluginActionModal",
  ];
  return ids.some((id) => {
    const el = $(id);
