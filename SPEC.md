...
**One exception to "deletes every declared path": `ro-crate-metadata.json` itself.** `deletePluginOutputs()` (`main.js`) skips it specifically, even though `ro-crate-json-output` declares it like any other output. That plugin has no gating option at all — it rewrites the file every build regardless — so deleting it first, before the rest of the pipeline even runs, achieved nothing except removing the one file `generic-input` needs to read for reconciliation (§6.1a). With the setting **on by default**, that meant every build against an existing crate silently took the from-scratch path instead, with no error and no warning distinguishing it from an empty folder — reproduced against the real F2F crate (§6.1a's numbers): delete the file first, rebuild, get exactly 465 entities back, indistinguishable from the original bug. `ro-crate-preview.html`/`.xlsx` aren't given the same exception — those *do* have gating options (`makeHtml`/`makeXlsx`), so deleting a stale one before a build that has the option off is the setting doing its actual job.
...
**Graph assembly.** `buildCrate(filesWithMeta, config, log, opts)` initialises an `ROCrate` — from scratch, or from the folder's own existing crate when there is one (§6.1a) — with the `ldac`, `pcdm`, `custom`, and `AUSTLANG` contexts, applies the profile-derived root dataset, emits or reconciles folder and file entities, and rewrites structural hash-ids (`#Dyirbal`) to `arcp://` form on export.

Top-level folders are emitted one of two ways, when there is no existing crate to reconcile against instead (§6.1a):
...
### 6.1a Persist and reconcile, not rebuild

**A build must never discard a folder's existing crate.** If `ro-crate-metadata.json` is already there, a build reconciles the current folder scan against it — updates, doesn't replace. This is a correctness requirement, not an optimisation: a crate is often the product of real curation (interview objects, people, places, transcripts — structure and metadata no folder scan could ever reconstruct from filenames alone), and a tool that silently throws that away the moment someone clicks Build is not a safe tool to run against a completed collection. It was possible, before this section existed, to open a folder holding a 764-entity curated crate, click Build, and end up with a 465-entity crate containing nothing but a bare file listing — bug, not a feature; this is what closes it.

**What "reconcile" means concretely**, in `buildCrate`'s `opts.existingJson` path (`generic-input`'s `buildCrate(ctx)` reads `ro-crate-metadata.json` from the folder — via `readJsonFromFolder`, not a new mechanism — and passes its parsed JSON through as `opts.existingJson` whenever the file exists):

- The crate starts from `loadCrateFromJson(existingJson)`, not an empty `ROCrate` — every entity the existing crate already has is present from the first line, not re-derived.
- **Every file the scan finds that already has a matching entity (`crate.getEntity(file.id)`, `file.id` being the file's folder-relative path) is left completely alone.** Not re-created, not reset to blank stubs, not stripped of whatever custom properties it carries. This is the core guarantee: an entity already in the crate is never a rebuild's business.
- **A physical file with no matching entity is new, and is never added without asking first.** Before `buildCrate` is even called, `generic-input` diffs the scanned files against the existing crate's own `@id`s and, if any are new, blocks the build on a confirmation modal (`c2c-plugins/src/generic-input/new-files-confirm.js`) — a checkbox file tree, everything checked by default, a folder's checkbox reflecting whether all/some/none of its files are checked. Only the confirmed subset is passed into `buildCrate` at all; an unchecked file is simply not part of this build's file list, so it isn't added — no different from a file the scan never saw. There's no persisted "seen and declined" state, so the same prompt reappears next build for anything still unconfirmed, unless it's removed from the folder. Dismissing the modal outright (rather than confirming a selection, even an empty one) cancels the whole build with an error, since a dismissal made no explicit choice at all — treating it as "add nothing" would be a silent guess in the other direction.

  For whatever *is* confirmed: if the existing graph already has an entity at the id `filesWithMeta` would have generated for its top-level folder (the same `#<foldername>` convention `addFolderEntities` uses for a from-scratch build), the new file's File entity is attached there (`hasPart`); otherwise — the common case, since a curated crate's own ids rarely follow that convention — it's attached directly to the root dataset's `hasPart`, logged as a warning, since "found a new file, don't know where it really belongs" still needs a human's attention even after the file itself was confirmed.
- **A File entity whose file no longer exists in the folder is never deleted.** It's left in the graph exactly as it was, and a warning is logged naming it. Deleting on a rebuild's own initiative is the same mistake as replacing wholesale, just scoped to one entity instead of the whole graph — "missing from disk" might mean deleted, or might mean temporarily unmounted, renamed, or on a drive that isn't attached this session; only a person curating the collection can tell the difference, so the tool reports and waits.
- **Root dataset fields from Describe/the profile still apply**, the same `Object.assign(crate.rootDataset, config.rootDataset)` as a from-scratch build — but onto the entity the existing crate already populated, so an existing field `config.rootDataset` doesn't mention (a hand-set `datePublished`, `conformsTo`, `license`, …) is simply never touched. Only the fields Describe/the profile actually supplied get overwritten — the same "the explicit answer wins" rule `xlsx-crate-input`'s own root-merge already follows (§7.2).
- Nothing about `RepositoryObject`/`RepositoryCollection`/`Person`/`Place`/… entities changes at all in the reconcile path — no folder-object invention (`addFolderEntities` doesn't run), no touching an entity the scan didn't ask about. The existing graph's own structure is the structure.
- Calling `addContext` with the same mapping on every build (fresh or reconciled) would otherwise grow `@context` by one duplicate, content-identical entry per rebuild forever, since `ROCrate.addContext` dedupes by object *reference*, not content, and a rebuild always constructs a fresh literal. `ensureContext` (crate.js) checks for a content-equivalent entry first, so reconciling repeatedly is actually idempotent rather than merely non-destructive.

**What this deliberately does not do.** It doesn't attempt to relocate a new file into the "right" curated container by any heuristic beyond the one folder-name convention check above — guessing wrong would be worse than a clearly-flagged root attachment. It doesn't second-guess whether a missing file was really deleted. It doesn't merge in a *different* crate's entities (that's `xlsx-crate-input`'s separate, explicit `additional-ro-crate-metadata.xlsx` job, §7.2) — this is about the folder's own crate, the one this same build is about to overwrite with its own output. And it's specific to `generic-input`; `docx-input` builds its crate a different way entirely (§7.1) and isn't in scope here.

**Interaction with §5.3's prefill.** That section's "read of the root entity only" describes a *different* read — the Describe step peeking at whatever crate source is newest, purely to fill in form fields, before Build has even run. This section is about what Build itself does with the folder's crate once Describe is done and the pipeline actually runs. The two aren't in tension: prefill still only touches root fields for the wizard; reconciliation is what makes the eventual Build safe to run against a crate that prefill merely glanced at.

**Interaction with "Delete plugin output before rebuilding."** This setting is on by default and, naively, deletes every plugin-declared output — including `ro-crate-metadata.json` itself — before the folder is even scanned. Reconciling against a file that's already been deleted just looks like a from-scratch build with no existing crate, silently, with nothing in the log to say why. §4.6 documents the one exception this requires: `deletePluginOutputs()` specifically skips `ro-crate-metadata.json`, since nothing gates it anyway (it's rewritten every build regardless), so deleting it first was pure loss with no upside.
...
**It reconciles rather than rebuilds when the folder already has a crate.** Before calling `buildCrate`, it reads `ro-crate-metadata.json` from the folder (if present) and passes it through as `opts.existingJson` — see §6.1a for what that changes. This is the same "don't compete with what's already there" instinct as the `structureFromMetadata` case above, just against the folder's own prior output instead of a spreadsheet. Any file the scan found with no matching entity blocks the build on a checkbox-tree confirmation modal before `buildCrate` is even called — §6.1a again, this is the part of reconciling that isn't automatic.
...
**Choosing a new folder mid-build cancels the build in flight**, rather than leaving it to run to completion against a folder the UI has since moved on from. There's no `AbortController` threaded through the pipeline — cancellation instead is a generation counter, `buildGeneration` in `main.js`. `run()` captures its own generation when it starts and checks it after every `await` (permission check, folder walk, `processFolder`/`runPipeline`, preview read); `pickFolder()` calls `cancelRunningBuild()` right after a new folder handle is obtained, which bumps the counter, logs a cancellation notice, and resets the build button and progress bar to idle. A stale `run()` call whose generation no longer matches simply stops touching the log, stats, progress bar, and buttons — it doesn't actually interrupt whatever plugin work is still in flight (a hook mid-write finishes writing), but that work is now invisible and harmless since the UI has already moved to the new folder.
