...
- **A physical file with no matching entity is new** — a File entity is added for it. Where it attaches: if the existing graph already has an entity at the id `filesWithMeta` would have generated for its top-level folder (the same `#<foldername>` convention `addFolderEntities` uses for a from-scratch build), the new file is attached there (`hasPart`); otherwise — the common case, since a curated crate's own ids rarely follow that convention — it's attached directly to the root dataset's `hasPart`, and logged clearly as a warning, since "found a new file, don't know where it really belongs" is exactly the kind of thing that needs a human's attention, not a silent guess.
